...
# Added new function for subprojects
def listSubProjects(BASE_DIR,group):
  projects = []
  if os.path.exists("%s/repositories/%s/_manifests" % (BASE_DIR, group)):
    projects.append("%s" % group)
  else:
    for subproj in os.listdir("%s/repositories/%s" % (BASE_DIR, group)):
      for project in listSubProjects(BASE_DIR,"%s/%s" % (group, subproj)):
...
def listProjects(BASE_DIR):
  ...
  for group in os.listdir("%s/repositories" % BASE_DIR):
    for project in listSubProjects(BASE_DIR, group):
      projects.append(project)
#    if os.path.exists("%s/repositories/%s/_manifests" % (BASE_DIR, group)):
...
#    else:
#      for subproj in os.listdir("%s/repositories/%s" % (BASE_DIR, group)):
#        for project in os.listdir("%s/repositories/%s/%s" % (BASE_DIR, group, subproj)):
#          projects.append("%s/%s/%s" % (group, subproj, project))
  return projects
...
BASE_DIR = '/srv/gitlab/data/gitlab-rails/shared/registry/docker/registry/v2'
